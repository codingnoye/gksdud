...
설정은 **일반 / 대소문자 / 특수문자 / gksdud** 탭으로 나뉩니다.
- **특수문자**: 한글에서 `⌥+문자`, `⌥⇧+문자`를 영어처럼 입력하는 실험적 모드와 Option 문자 입력 차단 모드를 선택할 수 있습니다. 기본값은 ‘없음’입니다. 차단은 한글·영어 모두에 적용하며 편집 키와 Command·Control 단축키는 유지합니다.
- **업데이트**: 하루 한 번 최신 정식 릴리스를 확인합니다. 새 버전이 있으면 메뉴의 ‘업데이트 가능’과 gksdud 탭의 화살표 배지로 표시하고, 릴리스 노트의 ‘요약’을 보여줍니다. ‘업데이트 하기’를 누르면 새 앱을 다운로드하고 서명을 검증한 뒤 교체·재실행합니다.
...
설정의 **대상 키보드 설정**에서 키보드마다 적용 여부를 선택할 수 있습니다.
- **기본값**: 새로운 키보드와 `Default`인 키보드에 적용합니다. 처음에는 켜져 있습니다.
- **Off / Default / On**: 적용하지 않기 / 기본값 따르기 / 항상 적용입니다. `On`과 `Off`가 기본값보다 우선합니다.
- 연결이 끊긴 키보드도 회색 이름으로 남고 설정을 기억합니다. 고유 정보가 없는 동일 모델은 설정을 공유할 수 있습니다.
- 반복해서 적용에 실패하면 메뉴바 아이콘 좌하단에 주황색 느낌표가, 활성화 아래에 오류 안내가 표시됩니다. 복구되면 자동으로 사라집니다.
...
- **한영 전환은 기본 입력기를 사용합니다.** 한영 키는 키 매핑과 시스템 전환 단축키로 처리합니다. 선택 기능인 특수문자 모드는 실제 문자 키가 눌렸을 때 영어 입력 소스로 잠깐 전환해 입력한 뒤 원래 한글 입력 소스로 복귀합니다.
